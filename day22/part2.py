...
def fall(id: int, falling: Set[int]) -> Set[int]:
    ret = set(falling)
    for supported in supporting.get(id, set()):
        if supported_by[supported] <= ret:
            ret.add(supported)
            ret = ret | fall(supported, ret)
    return ret

total = 0
for brick in bricks:
    falling = fall(brick.id, set([brick.id]))
    print(brick.id, falling)
    total += len(falling) - 1
print(total)